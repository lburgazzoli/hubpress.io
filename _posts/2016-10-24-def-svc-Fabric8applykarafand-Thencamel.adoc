...
IMPORTANT: If you deploy on OpenShift you need to grant edit role to the service account `oc policy add-role-to-user view system:serviceaccount:$(oc project -q):default -n $(oc project -q) `
